...
### Layer 3 — Self-frozen behavioral golden master (FROZEN at end of Phase 2)
...
> **Status (Phase 2 complete):** frozen. The scripted v110 demo
> (`tests/fixtures/parity.lmp`, generated by `tools/gen_demo.py`) replays to
> canonical world-state checksum **`a00552bbf22274a2`**
> (`tests/fixtures/parity.checksum`) and indexed-frame hash
> **`3e61b0f0c5dfd943`** (`tests/fixtures/parity.frame`). Both were frozen under
> the stub build and reproduce byte-identically under the SDL2 backend. A
> dedicated `palette-lut` gate covers the SDL index→ARGB conversion, which frame
> hashes do not exercise. Boot-smoke evidence: `docs/oracle/phase2-boot-frame.png`.
